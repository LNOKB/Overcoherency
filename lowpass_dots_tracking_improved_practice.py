...
FEEDBACK_SEC = 0.6  # フィードバック表示時間（秒）

fb = visual.TextStim(
    win, text='', pos=(0,0), color='white', height=0.12
)
...
for t_idx in range(N_TRIALS):
    ...
    # 等しいなら False／違えば True（未回答は不正解にする）
    correct = (resp_deg is not None) and (resp_deg != DIR_DEG)

    # --- 方向（Up/Down）のフィードバック ---
    event.clearEvents()
    fb.text = '正解' if correct else ('時間切れ' if resp_deg is None else '不正解')
    fb.setColor('lime' if correct else ('yellow' if resp_deg is None else 'red'))
    fb_clock = core.Clock()
    while fb_clock.getTime() < FEEDBACK_SEC:
        fb.draw()
        win.flip()
